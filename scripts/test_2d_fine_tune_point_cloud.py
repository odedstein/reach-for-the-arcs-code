...
V, F, P, N = rfta.reach_for_the_arcs(U, S,
    fine_tune_iters=5, max_points_per_sphere=1,
    parallel=False, rng_seed=rng_seed,
    debug_Vgt=poly, debug_Fgt=EC,
    return_point_cloud=True, verbose=True)
Vf, Ff, Pf, Nf = rfta.reach_for_the_arcs(U, S,
    fine_tune_iters=5, max_points_per_sphere=3,
    parallel=False, rng_seed=rng_seed,
    debug_Vgt=poly, debug_Fgt=EC,
    return_point_cloud=True, verbose=True)
...
plot_edges(V, F, 'g-')
plot_pts(P, N, f'g')
plt.title('Reach for the Arcs - no additional pts')
...
plot_edges(Vf, Ff, 'g-')
plot_pts(Pf, Nf, f'g')
plt.title('Reach for the Arcs - with additional pts')
